...
## Status: VERCEL ENV-VAR BUILD STEP READY (tasks #12 + #13 + #15 done) — the panel deploys via `node build.mjs` (Output `dist/`), which bakes the contract addresses into `dist/config.js` from the Vercel env vars `INTERLOCK_ADDRESS` / `VAULT_ADDRESS` (committed `frontend/config.js` holds the defaults). Headless-verified the BUILT dist against the live pair: RUNNING @ 142%, armed, identity chip, 3 incidents. Real bug caught by QA: **studionet contract lookup is CASE-SENSITIVE on the exact deployed address string** — config must carry the deployed casing verbatim (lowercasing an address → `gen_call: Contract … not found`). Next: user updates Vercel settings (Build Command + Output Directory + 2 env vars) & redeploys, then #9 README + #14 optional real-MetaMask signer.

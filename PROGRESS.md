...
- **Deploy to Vercel:** Project → Settings → General: **Root Directory = `frontend`**, Framework Preset = Other, **Build Command = `node build.mjs`**, **Output Directory = `dist`**. Then Project → Settings → Environment Variables — add for Production (and Preview) exactly as deployed:
